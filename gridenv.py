...
class GridWorldRand:
    def __init__(self, N, rand, side_len = 10, seed=1, eps_offset = 0) -> None:
        np.random.seed(seed)
        dim = 2
        self.side_len = side_len
        spacing = 2*side_len/(2*N + 1)
        ...
        for idx_x in range(N):
            for idx_y in range(N):
                #center_x = 2*(2*spacing + 2*spacing*idx_x - side_len/2)
                #center_y = 2*(2*spacing + 2*spacing*idx_y - side_len/2)
                perturb_x = np.maximum(rand*2*(np.random.rand()-0.5), - 0.45*spacing) 
                perturb_y = np.maximum(rand*2*(np.random.rand()-0.5), - 0.45*spacing) 
                p_min = [spacing*1.0 -perturb_x + idx_x*2*spacing - side_len, spacing*1.0 -perturb_y + idx_y*2*spacing - side_len]
                p_max = [spacing*2.0 +perturb_x + idx_x*2*spacing - side_len, spacing*2.0 +perturb_y + idx_y*2*spacing - side_len]
                self.obstacles.append(HPolyhedron.MakeBox(p_min, p_max))
                self.inflated_obstacles.append(HPolyhedron.MakeBox(np.array(p_min)-eps_offset, np.array(p_max)+eps_offset))
        #shell_Verts =sorted_vertices(VPolytope(self.iris_domain)).T
        self.cfree_polygon = shapely.Polygon(shell =postappend(sorted_vertices(VPolytope(self.iris_domain)).T), 
                                             holes=[postappend(sorted_vertices(VPolytope(obs)).T) for obs in self.obstacles])

        self.cfree_offset_polygon = shapely.Polygon(shell =postappend(sorted_vertices(VPolytope(self.iris_domain)).T), 
                                             holes=[postappend(sorted_vertices(VPolytope(obs)).T) for obs in self.inflated_obstacles])

        self.offset_polys = {f"{eps_offset}": self.cfree_offset_polygon}
    ...
    def sample_cfree_offset(self, n):
        points = []
        while len(points) < n:
            point = np.random.uniform(low=[-self.side_len, -self.side_len], high=[self.side_len, self.side_len])
            if self.point_in_cfree_offset(point):
                points.append(point)
        return np.array(points)
    ...
    def col_handle_offset(self, pt):
        for r in self.inflated_obstacles:
            if r.PointInSet(pt):
                return True
        return False
    ...
    def point_in_cfree_offset(self, pt):
        if not self.col_handle_offset(pt):
            return True
        # for r in self.cfree_pieces:
        #     if r.PointInSet(pt):
        #         return True
    ...
    def plot_HPoly(self, ax, HPoly, color = None, zorder = 1):
        v = sorted_vertices(VPolytope(HPoly)).T#s
        v = np.concatenate((v, v[0,:].reshape(1,-1)), axis=0)
        if color is None:
