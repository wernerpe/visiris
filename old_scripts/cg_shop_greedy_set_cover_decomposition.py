...
for world_name in ["cheese102.instance.json"]:#, "fpg-poly_0000000060_h1.instance.json","fpg-poly_0000000070_h1.instance.json", "srpg_iso_aligned_mc0000172.instance.json"]: #["srpg_iso_aligned_mc0000172.instance.json"]:#"fpg-poly_0000000060_h1.instance.json","fpg-poly_0000000070_h1.instance.json"]:#["srpg_iso_aligned_mc0000172.instance.json"]:#"fpg-poly_0000000300_h5.instance.json","srpg_iso_aligned_mc0000172.instance.json","fpg-poly_0000000060_h1.instance.json","fpg-poly_0000000070_h1.instance.json"]:
    for approach in [0]:
        for seed in [1]:
            ...
            def vgraph_builder(points):
                ...
                for i in tqdm(range(n)):
                    ...
                    for j in range(len(points[:i])):
                        ...
                        # else:
                        if np.linalg.norm(point - other) <=4 and  vis_reg(point, other, world, []):
                            adj_mat[i,j] = adj_mat[j,i] = 1
            ...
            loggerccd = CliqueApproachLogger(world, world_name,f"_{ap_names[approach]}_6ptcl_" +strftime("_%m%d%H%M%S_", gmtime()), seed, N, eps, plt_time = 10)
