...
class Logger:
    ...
    def log(self, vs: VisSeeder, iteration):
        ...
        int_poly = list(self.world.offset_polys.values())[0].buffer(0).intersection(union_of_Polyhedra)
...
class CliqueApproachLogger:
    ...
    def log(self, vs: VisCliqueDecomp, iteration):
        ...
        #write summary
        if vs.regions[0].ambient_dimension() ==2:
            shapely_regions = []
            for r in vs.regions:
                verts = sorted_vertices(VPolytope(r))
                shapely_regions.append(shapely.Polygon(verts.T))
...
class LoggerClique3D:
    ...
    def log(self, vs: VisCliqueDecomp, iteration):
        ...
        #self.world.plot_regions(vs.regions)
        ...
        summary=[f"-------------------------------------------\n",
                 ...
                 f"number of regions total {len(vs.regions)}\n"
                 f"tstep {t_step:.3f}, t_total {t_total:.3f}\n"
                 f"tsample {t_sample:.3f}, t_visgraph {t_visgraph:.3f}, t_ccv {t_ccv:.3f}, t_regions {t_regions:.3f}\n"
                 f"number of regions total {len(vs.regions)}\n"
                 f"coverage {coverage:.4f}\n"]
